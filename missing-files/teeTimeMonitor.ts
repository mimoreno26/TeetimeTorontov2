...
class TeeTimeMonitor {
  ...
  private async checkFilterForMatches(job: MonitoringJob): Promise<void> {
    try {
      ...
      const availableTimes = await this.getAvailableTeeTimesFromAPI(filter);
  ...
  private async getAvailableTeeTimesFromAPI(filter: any): Promise<TeeTimeSlot[]> {
  ...
  private async saveMonitoringJob(job: MonitoringJob): Promise<void> {
    ...
    console.log(`💾 Saved monitoring job: ${job.id}`);
  }

  private async updateMonitoringJob(job: MonitoringJob): Promise<void> {
    // TODO: Implement Supabase update
  }

  private async deactivateMonitoringJob(jobId: string): Promise<void> {
