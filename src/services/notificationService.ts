...
class NotificationService {
  ...
  async canSendNotification(userId: string, filterId: string): Promise<{
    ...
    if (activity && activity.notificationCount >= this.maxNotificationsPerCooldown) {
  ...
  private async getUserActivity(_userId: string): Promise<UserActivity | null> {
    // TODO: Implement Supabase query
    return null;
  }

  private async saveUserActivity(_activity: UserActivity): Promise<void> {
    // TODO: Implement Supabase insert/update
  }

  private async setCooldown(_userId: string, _cooldownUntil: Date): Promise<void> {
  ...
  private async getRecentNotifications(_userId: string, _filterId: string, _timeWindow: number): Promise<NotificationHistory[]> {
  ...
  private async logNotification(
    ...
    _courseId: string,
    _timeSlot: string,
    _type: 'sms' | 'email' | 'push',
    _status: 'sent' | 'delivered' | 'failed'
  ): Promise<void> {
